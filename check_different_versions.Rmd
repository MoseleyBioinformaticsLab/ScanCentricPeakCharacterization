...
curr_files <- dir("mspl_files_defaults_fullrun_X2017.07.14.14.54.20_0.0.281", full.names = TRUE, recursive = TRUE)
...
```{r final_set}
final_data <- lapply(c(prev_files$mspl_files_defaults_fullrun_X2017.07.11.16.19.33_0.0.183[1], curr_files$mspl_files_defaults_fullrun_X2017.07.17.09.09.13_0.0.287[1]), function(in_file){
  tmp_env <- new.env()
  load(in_file, envir = tmp_env)
  tmp_env
})
...
```{r load_finals}
prev_mspl <- grep("mspl", names(prev_files), value = TRUE)
curr_mspl <- grep("mspl", names(curr_files), value = TRUE)

prev_final <- lapply(prev_files[[prev_mspl]], function(in_file){
  tmp_env <- new.env()
  load(in_file, envir = tmp_env)
  tmp_env
})

curr_final <- lapply(curr_files[[curr_mspl]], function(in_file){
  tmp_env <- new.env()
  load(in_file, envir = tmp_env)
  tmp_env
})
...
```{r compare_finals}
for (ifile in seq(1, length(prev_final))) {
  print(all.equal(prev_final[[ifile]]$peak_finder$correspondent_peaks$master_peak_list$normalization_factors,
            curr_final[[ifile]]$peak_finder$correspondent_peaks$master_peak_list$normalization_factors))
  print(all.equal(prev_final[[ifile]]$peak_finder$correspondent_peaks$master_peak_list$master,
            curr_final[[ifile]]$peak_finder$correspondent_peaks$master_peak_list$master))
}
```


### Sample-Sample Correspondence

```{r cor_files}
cor_files <- curr_files$mspl_files_defaults_fullrun_X2017.07.14.14.54.20_0.0.281[1:2]
...
multisample$sd_fit_function <- cor_mpl[[1]]$sd_fit_function
multisample$sd_predict_function <- cor_mpl[[1]]$sd_predict_function
