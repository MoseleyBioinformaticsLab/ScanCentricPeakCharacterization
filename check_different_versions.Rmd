...
curr_files <- dir("mspl_files_defaults_fullrun_X2017.07.10.13.08.55_0.0.268", full.names = TRUE, recursive = TRUE)
...
corresp_data <- lapply(c(prev_files[[use_files]][1], curr_files[[use_files]][1]), function(in_file){
  tmp_env <- new.env()
  load(in_file, envir = tmp_env)
  tmp_env
})
...
```{r corresp_info}
collapse_data <- lapply(c(prev_files[[use_files]][3], curr_files[[use_files]][3]), function(in_file){
  tmp_env <- new.env()
  load(in_file, envir = tmp_env)
  tmp_env
})
