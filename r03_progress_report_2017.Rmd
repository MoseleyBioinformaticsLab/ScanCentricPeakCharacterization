...
data_sparcity <- data.frame(perc.sparse = c(single_sparseness, double_sparseness, triple_sparseness) * 100,
                            n_sample = c(1, 2, 3))
```

```{r report_sparcity}
knitr::kable(data_sparcity, digits = 0)
```
...
sparsity_figure <- ggplot(frac_notna, aes(x = nonzero_frac)) + geom_histogram() + scale_y_log10() + xlab("fraction non-zero")
sparsity_figure
```
...
of non-zero entries across samples is higher than previously observed.
...
doc <- addSlide(doc, slide.layout = "Content with Caption")
doc <- addPlot(doc = doc, fun = pritn, x = sparsity_figure, vector.graphic = TRUE)
