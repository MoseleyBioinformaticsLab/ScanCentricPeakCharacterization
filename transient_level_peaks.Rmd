...
peak_lists <- lapply(multi_scans, function(x){
  x$get_peak_info(calc_type = "basic")
})

peaks_n <- vapply(peak_lists, nrow, numeric(1))

init_multiplier <- 10
master_peaks <- rep(NA, max(peaks_n) * init_multiplier)
...
master_peaks[1:n_in1] <- peak_lists[[1]]$ObservedMZ
...
for (iscan in seq(2, out_scan)) {
  ...
  if (n_new > 0) {
    fit_new <- (sum(is.na(master_peaks)) - n_new) >= 0
    ...
    which_na <- min(which(is.na(master_peaks)))
    new_loc <- seq(which_na, which_na + n_new - 1)
    master_peaks[new_loc] <- tmp_scan[peak_new, "ObservedMZ"]
