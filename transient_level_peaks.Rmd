...
scan_matrix <- as.matrix(scan_test)
scan_matrix2 <- scan_matrix
scan_matrix2[,2] <- log_with_min(scan_matrix[,2, drop = FALSE])

plot(scan_matrix2[,1], scan_matrix2[,2])
```

```{r do_peak_picking, eval = FALSE}
out_peak <- locate.peaks(scan_matrix2, peak.method = "parabola")
```
