...
```{r integrate_points}
integrate_corners <- function(peak_mz, peak_int, full_peak_loc, model_peak_loc){
...
integration_based_area <- function(mz_data, int_data, full_peak_loc, model_peak_loc, model_coeff, n_point = 100, log_transform = "log"){
  ...
  side_area <- integrate_corners(mz_data, int_data, full_peak_loc, model_peak_loc)
  
  c(Area = model_area + side_area)
}

area_integrate <- integration_based_area(mz_data$mz, mz_data$intensity, pracma_peak_index, peak_index, peak_model)
```
...
## Use Derivative for Center

We should be able to use the derivative of the parabola to find where the inflection
point is, regardless of how many points we have on each side. Lets write a function and 
test this idea.

Based on the equation for the parabola:
...
```{r center_by_derivative}
derive_peak_center <- function(x_in, model_coeff){
  peak_center <- ((-1 * model_coeff[2]) / (2 * model_coeff[3])) + mean(x_in)
  peak_center
}
...
y_fun <- function(x_in, model_coeff){
  out_val <- model_coeff[1] + model_coeff[2]*x_in + model_coeff[3]*(x_in^2)
  out_val
}

y_out <- y_fun(x, coeff_1)

# create some x's that don't have all of the original points
x2 <- seq(4, 10)
coeff_2 <- parabolic_fit(x2, y_out[3:9])
derive_peak_center(x2, coeff_2)
```
