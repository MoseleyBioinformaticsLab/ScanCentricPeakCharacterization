...
test_peak_points <- function(mz_peak, min_peak = 5){
  n_point <- nrow(mz_peak)
  if (n_point >= min_peak) {
    test_index <- lapply(seq(1, n_point - min_peak), function(start_loc){
      lapply(seq(start_loc + min_peak - 1, n_point), function(end_loc){
        c(start_loc, end_loc)
      })
    })
...
choose_peak_points <- function(rsq_results, min_rsq = 0.98){
  filter_rsq <- rsq_results[rsq_results[,4] >= min_rsq, ]
...
peak_index <- pracma_peak_index[peak_points[1]:peak_points[2]]
```
...
```{r center_intensity}
peak_center_intensity <- function(x, coefficients){
  center_x <- mean(x)
  center_int <- coefficients[1]
  c(center = center_x, intensity = center_int)
}

peak_center <- peak_center_intensity(mz_data$mz[peak_index], peak_model)
```
...
area_sum_points <- function(peak_mz, peak_intensity, zero_value){
  mean_diff <- mean(diff(peak_mz))
  sum((peak_intensity - zero_value) * mean_diff)
}
