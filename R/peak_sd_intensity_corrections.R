...
#' @param percentage  what percentage was it observed in
#'
#'
...
correct_variance <- function(observed_variance, percentage){
  if (percentage == 1) {
    return(observed_variance)
  }
  x <- qnorm(1 - percentage)
  A <- (x * psi(x)) / Z(x)
  B <- (psi(x) / Z(x))^2

  corrected_variance <- observed_variance / (1 + A - B)
...
#' @param percentage the percentage of total observations
#'
#' @references https://en.wikipedia.org/wiki/Truncated_normal_distribution
#'
...
correct_mean <- function(observed_mean, corrected_sd, percentage){
  if (percentage == 1) {
