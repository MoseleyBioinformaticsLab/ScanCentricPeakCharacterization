...
#' @export
get_scan_info = function(mzml_data){
...
#'   sc_raw = SCMzml$new(lipid_sample)
#'
...
SCMzml = R6::R6Class("SCMzml",
   ...
   #' @param rt_range the retention time to use for scans
   ...
   initialize = function(mzml_file,
     ...
     if (is.null(scan_range) && is.null(rt_range)) {
       ...
       self$scan_info = get_scan_info(self$mzml_data)
