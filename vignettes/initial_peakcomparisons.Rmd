...
xcal_found_peaks <- filter(xcal_mz_deeper, which == "mz", mz_diff <= 5e-4) %>% 
...
plot_peaks(filter(xcal_mz_deeper_missed, s_peak %in% 143), mz_avg, 5e-3)
plot_peaks(filter(xcal_mz_deeper_missed, s_peak %in% 216), mz_avg, 1e-2)
plot_peaks(filter(xcal_mz_deeper_missed, s_peak %in% 411), mz_avg, 1e-2)
```
...
```{r test_pracma_again}
test_mz <- filter(mz_avg, (mz >= 1486) & (mz <= 1486.25))
...
```{r plot_all_peaks}
ggplot(all_peaks, aes(x = mz, y = intensity)) + geom_point()
```

```{r plot_one_peak}
one_peak <- filter(all_peaks, (mz >= 1486.116) & (mz <= 1486.13))
ggplot(one_peak, aes(x = mz, y = intensity)) + geom_point()
```
...
```{r plot_scans}
ggplot(filter(all_scans, scan <= 10), aes(x = mz, y = intensity, color = as.factor(scan))) + geom_line() + geom_point()

ggplot(filter(all_scans, (scan > 10) & (scan <= 20)), aes(x = mz, y = intensity, color = as.factor(scan))) + geom_line() + geom_point()

ggplot(filter(all_scans, (scan > 20) & (scan <= 30)), aes(x = mz, y = intensity, color = as.factor(scan))) + geom_line() + geom_point()

ggplot(filter(all_scans, (scan > 30) & (scan <= 40)), aes(x = mz, y = intensity, color = as.factor(scan))) + geom_line() + geom_point()
```
