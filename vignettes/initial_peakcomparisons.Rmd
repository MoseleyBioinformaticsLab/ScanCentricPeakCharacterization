...
(sum(missed_peaks$intensity) / sum(xcal_peaks$intensity)) * 100
```
...
p_points <- pracma_findpeaks(test_mz)
tmp_locs <- pracma::findpeaks(test_mz$intensity)

lapply(seq(1, nrow(tmp_locs)), function(x){
  mean(test_mz$mz[seq(tmp_locs[x, 3], tmp_locs[x, 4])])
})
```
