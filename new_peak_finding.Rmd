...
```{r}
window_1_tiled <- run_peak_picking_in_reduced(reduced_windows[1], mz_windows, tiled_windows)
window_1_tmp <- window_1_tiled$tiled
reduce(window_1_tmp[mcols(window_1_tmp)$peak_count > 0])


ggplot(window_1_tiled$plot, aes(x = mz, y = intensity, color = type)) + geom_point()

window_2_tiled <- run_peak_picking_in_reduced(reduced2, mz_windows, tiled_windows)
window_2_tmp <- window_2_tiled$tiled
reduce(window_2_tmp[mcols(window_2_tmp)$peak_count > 0])

ggplot(window_2_tiled$plot, aes(x = mz, y = intensity, color = type)) + geom_point()

sub_peaks <- purrr::map_df(unique(sub_data@elementMetadata$scan), function(in_scan){
  print(in_scan)
  tmp_range <- sub_data[sub_data@elementMetadata$scan %in% in_scan, ]
  mz_data <- tmp_range@elementMetadata
  mz_data$log_int <- log(mz_data$intensity + 1e-8)
  possible_peaks <- pracma::findpeaks(mz_data$intensity, nups = 2)
  ...
  peaks <- purrr::map_df(seq(1, n_peak), function(in_peak){
    ...
    "!DEBUG Peak `in_peak`"
    peak_loc <- seq(possible_peaks[in_peak, 3], possible_peaks[in_peak, 4])
    out_peak <- get_peak_info(mz_data[peak_loc, ], peak_method = peak_method, min_points = min_points)
    out_peak$n_point <- length(peak_loc)
    out_peak$mz_width <- max(mz_data[peak_loc, "mz"]) - min(mz_data[peak_loc, "mz"])
    out_peak$points <- I(list(peak_loc))
    out_peak$scan <- in_scan
    out_peak
  })
  peaks
})

sub_peaks$mz <- sub_peaks$ObservedMZ
sub_peak_windows <- mz_points_to_windows(sub_peaks, tiled_windows@metadata$point_multiplier)

peaks_in_tiles <- countOverlaps(sub_tiles, sub_peak_windows)

tiled_windows2 <- create_mz_windows(mz_model, use_range = c(148, 1600), window_size = 2, delta = 1)
tiled_windows2 <- count_overlaps(tiled_windows2, mz_windows)
test1 <- subsetByOverlaps(tiled_windows2, reduced_windows[1])
test1_data <- as.data.frame(mcols(test1))

test1_points <- subsetByOverlaps(mz_windows, reduced_windows[1])

pracma::findpeaks(test1_data$count_ratio, nups = 2)
tmp_data <- as.data.frame(mcols(tiled_counts$window_counts))
use_tiles <- (tmp_data$mz_start > 1317.199) & (tmp_data$mz_end < 1317.23)
sub_region <- tiled_counts$window_counts[use_tiles]

tmp_data_region <- tmp_data[use_tiles, ]
pracma::findpeaks(tmp_data_region$overlap_ratio)

tmp_data_sliding <- as.data.frame(mcols(window_counts$window_counts))
use_sliding <- (tmp_data_sliding$mz_start > 1317.199) & (tmp_data_sliding$mz_start < 1317.23)

tmp_data_sliding_region <- tmp_data_sliding[use_sliding, ]

```

```{r sub_windows}
split_sub_windows <- function(overlap_counts, nups = 2){
  window_data <- mcols(overlap_counts)
  #has_nonzero <- window_data$nonzero_counts > 0
  #window_data <- window_data[has_nonzero, ]
  #overlap_counts <- overlap_counts[has_nonzero]
  sub_peaks <- pracma::findpeaks(window_data$overlap_ratio, nups = nups)
  if (!is.null(sub_peaks)) {
    peak_list <- IRanges(start = start(overlap_counts)[sub_peaks[, 3]],
                         end = start(overlap_counts)[sub_peaks[, 4]])
    mcols(peak_list) <- data.frame(overlap_ratio = window_data$overlap_ratio[sub_peaks[, 2]])
    
    n_peaks <- length(peak_list)
    
    if (n_peaks > 1) {
      end(peak_list)[n_peaks] <- end(overlap_counts)[sub_peaks[n_peaks, 4]]
    }
    
  } else {
    peak_list <- IRanges(start = min(start(overlap_counts)),
                         end = max(start(overlap_counts)))
    mcols(peak_list) <- data.frame(overlap_ratio = max(mcols(overlap_counts)$overlap_ratio))
  }
  peak_list
}

find_sub_windows <- function(windows, original_counts, nups = 2){
  mcols(windows) <- data.frame(overlap_ratio = 0)
  split_windows <- split(windows, seq(1, length(windows)))
  
  for (iwindow in seq_len(length(split_windows))) {
    print(iwindow)
    split_windows[[iwindow]] <- split_sub_windows(
      subsetByOverlaps(original_counts, split_windows[[iwindow]]), nups = nups
    )
  }
  
  unlist(split_windows)
}

sub_windows <- find_sub_windows(reduced_windows, window_counts$window_counts)
mcols(sub_windows) <- list(mcols(sub_windows), peak = seq(1, length(sub_windows)))
```

```{r extract_points}
reduced_windows_points <- mergeByOverlaps(window_counts$nonzero_points, sub_windows)

split_points <- split(reduced_windows_points, reduced_windows_points$peak)

use_peaks <- c(1, 2, 9, 1000, 1300, 1500)
...
```{r check_n_scans, fig.keep = "all"}
n_scans <- purrr::map_int(all_peaks, function(x){
  length(unique(x$scan))
})

ggplot(data.frame(n_scans = n_scans), aes(x = n_scans)) + geom_histogram(bins = 30)

plot_0 <- c(18, sample(which(n_scans == 0), 4))
...
OK, have some cases with just single points in them.

```{r check_duplicate_scans}
has_dup <- which(purrr::map_lgl(all_peaks, function(in_peak){
  sum(duplicated(in_peak$scan)) > 0
}))

plot_dup <- unique(c(7, sample(has_dup, 4)))

for (ipeak in plot_dup) {
  p <- ggplot(as.data.frame(split_points[[ipeak]]), aes(x = mz, y = log10(intensity), color = as.factor(scan))) + geom_point() + geom_line() + theme(legend.position = "none")
  print(p)
}
```

So, there are probably two types of things. 

**Peak 7**, where there is nothing really, and should just sort of be dropped. And then side things, like **Peak 967**.
