...
self$reduce_signal_regions()
```

## Working with splitting sub-regions

```{r split_sub_regions}
library(furrr)
plan(multiprocess)
signal_regions <- self$peak_regions$peak_regions
mz_point_regions <- self$peak_regions$mz_point_regions
tiled_regions <- IRanges::subsetByOverlaps(self$peak_regions$tiled_regions, signal_regions)
...
split_peak_regions <- split_regions(signal_regions, mz_point_regions, tiled_regions)

has_only_40 <- purrr::map_lgl(split_peak_regions$peaks, function(x){
  length(x$scan) == 40
})

n_scan <- purrr::map_int(split_peak_regions$peaks, function(x){
  length(unique(x$scan))
})

has_dup_scans <- purrr::map_lgl(split_peak_regions$peaks, function(x){
...
normalize_peaks <- which(has_only_40)

nscan <- 40

peak_comparisons <- combn(nscan, 2)

peak_mz <- purrr::map_dfc(split_peak_regions$peaks[normalize_peaks], function(x){
  x$ObservedMZ[order(x$scan)]
})
peak_height <- purrr::map_dfc(split_peak_regions$peaks[normalize_peaks], function(x){
  log(x$Height[order(x$scan)])
})

scan_distances <- purrr::map_dbl(seq(1, nrow(peak_height)), function(in_scan){
  scan_peaks <- peak_height[in_scan, ,drop = FALSE]
  scan_peaks_matrix <- matrix(unlist(scan_peaks), nrow = nscan - 1, ncol = ncol(scan_peaks), byrow = TRUE)

  other_matrix <- as.matrix(peak_height[-in_scan, , drop = FALSE])
...
scan_norm_matrix <- matrix(unlist(peak_height[normalize_scan, , drop = FALSE]),
                             nrow = nscan, ncol = ncol(peak_height), byrow = TRUE)

diff_matrix <- as.matrix(peak_height) - scan_norm_matrix

diff_df <- as.data.frame(diff_matrix)

gather_diff <- tidyr::gather(diff_df, key = "peak", value = "diff") %>% dplyr::mutate(scan = rep(seq(1, 40), ncol(diff_df)),
                           peak_id = paste0(peak, ".", scan))

gather_mz <- tidyr::gather(peak_mz, key = "peak", value = "mz") %>% dplyr::mutate(scan = rep(seq(1, 40), ncol(diff_df)),
                           peak_id = paste0(peak, ".", scan))

gather_height <- tidyr::gather(peak_height, key = "peak", value = "height") %>% dplyr::mutate(scan = rep(seq(1, 40), ncol(diff_df)),
                           peak_id = paste0(peak, ".", scan))

height_ratio <- purrr::map_dfr(seq(1, nrow(peak_height)), function(in_row){
  (peak_height[in_row, ]) / max((peak_height[in_row, ]), na.rm = TRUE)
})
gather_ratio <- tidyr::gather(height_ratio, key = "peak", value = "ratio") %>% dplyr::mutate(scan = rep(seq(1, 40), ncol(diff_df)),
                           peak_id = paste0(peak, ".", scan))

gather_df <- dplyr::left_join(gather_diff, gather_mz) %>% dplyr::left_join(., gather_mz) %>% dplyr::left_join(., gather_height) %>% dplyr::left_join(., gather_ratio)

ggplot(dplyr::filter(gather_df), aes(x = mz, y = diff, color = as.factor(scan))) + geom_smooth(method = "loess", span = 1.5, se = FALSE) + theme(legend.position = "none")

ggplot(dplyr::filter(gather_df, scan == 1), aes(x = mz, y = diff, color = height)) + geom_point()
```

Based on these results, we still need to do M/Z aware normalization.
...
create_mz_windows <- compiler::cmpfun(create_mz_windows)
...
mz_points_to_windows <- compiler::cmpfun(mz_points_to_windows)
...
count_overlaps <- compiler::cmpfun(count_overlaps)
...
reduce_sliding_window <- compiler::cmpfun(reduce_sliding_window)
...
create_na_peak <- compiler::cmpfun(create_na_peak)
...
get_reduced_peaks <- compiler::cmpfile(get_reduced_peaks)
...
run_peak_picking_in_reduced <- compiler::cmpfun(run_peak_picking_in_reduced)
...
#save(sliding_windows, reduced_windows, all_regions2, tiled_windows, mz_windows, file = "peak_picking_reduced.RData")
```
...
## Actually Do Some Peak Characterization

```{r peak_characterization}
get_merged_peak_info <- function(in_points, peak_method = "lm_weighted", min_points = 4){
  use_points <- unlist(IRangesList(in_points))
...
get_merged_peak_info <- compiler::cmpfun(get_merged_peak_info)

regional_peak_characterization <- function(mz_points, min_peaks = 4, max_selection = 100, peak_method = "lm_weighted", min_points = 4){
  
  mz_points@elementMetadata$log_int <- log(mz_points@elementMetadata$intensity + 1e-8)
  mz_data <- mcols(mz_points)
  reduced_peaks <- purrr::map_df(unique(mz_data$scan), function(in_scan){
    get_reduced_peaks(mz_points[mz_data$scan %in% in_scan], peak_method = peak_method, min_points = min_points)
  })
  reduced_peaks <- reduced_peaks[!is.na(reduced_peaks$ObservedMZ), ]
  n_peaks <- nrow(reduced_peaks)
  if (n_peaks >= min_peaks) {
    reduced_peaks$peak <- seq(1, nrow(reduced_peaks))
  
    peak_points <- purrr::map(seq(1, nrow(reduced_peaks)), function(in_peak){
      use_points <- IRanges(start = reduced_peaks[in_peak, "points"][[1]], width = 1)
      use_scan <- reduced_peaks[in_peak, "scan"]
      subsetByOverlaps(mz_points[mz_data$scan %in% use_scan], use_points)
    })
    
    peak_info <- get_merged_peak_info(peak_points)
    peak_samples <- utils::combn(n_peaks, 3)
  
    if (ncol(peak_samples) > max_selection) {
      peak_samples <- peak_samples[, sample(ncol(peak_samples), max_selection)]
    }
    
    sampled_peaks <- purrr::map_df(seq_len(ncol(peak_samples)), function(in_sample){
      get_merged_peak_info(peak_points[peak_samples[, in_sample]])
    })
    ...
    peak_info$ObservedMZSD2 <- sd(reduced_peaks$ObservedMZ)
    peak_info$Height2 <- mean(reduced_peaks$Height)
    peak_info$HeightSD2 <- sd(reduced_peaks$Height)
  } else {
    peak_info <- create_na_peak()
    peak_info$ObservedMZSD <- NA
    peak_info$HeightSD <- NA
    peak_info$Log10HeightSD <- NA
    peak_info$ObservedMZSD2 <- NA
    peak_info$Height2 <- NA
    peak_info$HeightSD2 <- NA
  }
  
 peak_info
}
regional_peak_characterization <- compiler::cmpfun(regional_peak_characterization)
```

```{r run_subsamples}
n_regions <- length(all_regions2)
subsample_progress <- knitrProgressBar::progress_estimated(n_regions)
peak_subsamples <- purrr::map_df(seq_len(n_regions), function(in_region){
  #print(in_region)
  update_progress(subsample_progress)
  regional_peak_characterization(subsetByOverlaps(mz_windows, all_regions2[in_region]))
})

save(peak_subsamples2, sliding_windows, reduced_windows, all_regions2, tiled_windows, mz_windows, file = "peak_picking_reduced2.RData")
```
