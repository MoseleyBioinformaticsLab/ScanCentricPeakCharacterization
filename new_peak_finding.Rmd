...
sliding_window_iranges <- function(mz_intensity_df, use_range = NULL, mz_model = NULL, window = 10, delta = window / 10){
  ...
  mcols(windows) <- list(mz_start = mz_start, mz_end = mz_end, zero_counts = zero_overlaps, nonzero_counts = nonzero_overlaps, overlap_ratio = log10(nonzero_overlaps + 1) - log10(zero_overlaps + 1))
...
split_sub_windows <- function(overlap_counts, nups = 2){
  sub_peaks <- pracma::findpeaks(mcols(overlap_counts)$overlap_ratio, nups = nups)
  if (!is.null(sub_peaks)) {
    peak_list <- IRanges(start = start(overlap_counts)[sub_peaks[, 3]],
                         end = start(overlap_counts)[sub_peaks[, 4]])
    mcols(peak_list) <- data.frame(overlap_ratio = sub_peaks[, 1])
