...
sd_predict_function <- function(coef, x){
  SIRM.FTMS.peakCharacterization:::exponential_predict_zeroint(ceof, x)
}
```
...
rdata_save <- paste0("test_files/mspl_files_defaults_", make.names(Sys.time())) 
...
zip_files <- mclapply(use_files, function(ifile) {
  ...
  anal_ms <- AnalyzeMS$new(ifile, out_file = out_file, peak_finder = PeakFinder$new(
    raw_filter = filter_scans, sd_fit_function = sd_fit_function, sd_predict_function = sd_predict_function
  ))
  anal_ms$load_file()
