...
curr_files <- dir("mspl_files_defaults_fullrun_X2017.07.05.13.35.15", full.names = TRUE)

prev_data <- lapply(prev_files, function(x){
  tmp_env <- new.env()
  load(x, envir = tmp_env)
  pf <- tmp_env$peak_finder
  pf_scan_id <- grep("^scan", names(pf), value = TRUE)
  pf_scans <- lapply(pf_scan_id, function(in_id){
    pf[[in_id]]
  })
  names(pf_scans) <- pf_scan_id
  rm(tmp_env)
  pf_scans
})
...
curr_data <- lapply(curr_files, function(x){
  ...
  load(x, envir = tmp_env)
  pf <- tmp_env$peak_finder
  pf$multi_scan_peaklist$reset_scan_indices()
  pf_scan_id <- grep("^scan", names(pf), value = TRUE)
  pf_scans <- lapply(pf_scan_id, function(in_id){
    used_scans <- pf[[in_id]]
    pf$multi_scan_peaklist$scan_numbers()[used_scans]
  })
  names(pf_scans) <- pf_scan_id
  rm(tmp_env)
  pf_scans
...
check_res <- lapply(names(curr_data), function(in_sample){
  vapply(names(prev_data[[in_sample]]), function(in_scan){
    isTRUE(all.equal(sort(curr_data[[in_sample]][[in_scan]]), sort(prev_data[[in_sample]][[in_scan]])))
  }, logical(1))
})

names(check_res) <- names(curr_data)
check_res
